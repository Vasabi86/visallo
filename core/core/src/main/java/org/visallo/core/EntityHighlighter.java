...
public class EntityHighlighter {
    ...
    public static void transformToHighlightedText(InputStream text, OutputStream output, List<OffsetItem> offsetItems, EnumSet<Options> options) {
        try (
            ...
            if (options == null) {
                options = DefaultOptions; // EnumSet.noneOf(Options.class);
            }
    ...
    private static void writeBuffer(List<OffsetItem> started, List<OffsetItem> all, OutputStreamWriter out, char[] buffer, int offset, int len) throws IOException {
        ...
        /*if (strBuffer.contains("\n") && !started.isEmpty()) {
            ...
            int fromIndex = 0;
            while (fromIndex < strBuffer.length()) {
                int newLineIndex = strBuffer.indexOf("\n", fromIndex);
                if (newLineIndex == -1) {
                    out.write(StringEscapeUtils.escapeXml11(strBuffer.substring(fromIndex)));
                    break;
                } else {
                    out.write(StringEscapeUtils.escapeXml11(strBuffer.substring(fromIndex, newLineIndex)));
                    addClosingOffsetItems(out, started);
                    while (newLineIndex < strBuffer.length()) {
                        String str = strBuffer.substring(newLineIndex, newLineIndex + 1);
                        if (str.matches("\\s")) {
                            out.write(str);
                            newLineIndex++;
                        } else {
                    ...
                    addOffsetItems(out, started, all, started);
...
//        }
    ...
    private static boolean indexInsideCurrentBuffer(long index, int offset, int innerOffset, int len) {
        return index >= offset && (index - offset) < BUFFER_SIZE;// < (offset + innerOffset + len);
    }
...
//            if (opened.size() == 0 || opened.size() == 1 && opened.indexOf(item) == 0) {
//                int index = all.indexOf(item);
//                boolean noneNested = index != -1 && (
//                        index == (all.size() - 1) ||                   // Is Last, or
//                        item.getEnd() <= all.get(index + 1).getStart() // Next starts after this finishes
//                );
//                if (noneNested) {
//                    classes.add("iso");
//                }
